...
def get_elons_tweet():
    ...
    tweets = tweepy.Cursor(
        api.user_timeline, id="44196397", since=date.today(), tweet_mode="extended"
    ).items(1)

    # remove all invalid characters
    elons_last_tweet = [
        re.sub("[^A-Za-z0-9]+", " ", tweet.full_text) for tweet in tweets
    ]
...
def trade():
    ...
    if any(keyword in what_musk_said for keyword in keywords) and tweet_sentiment > 0:
        print("the madlad said it - buying some!")
        ...
        print(
            f"He did not say it, he said: {what_musk_said} - OR sentiment was not positive"
        )
